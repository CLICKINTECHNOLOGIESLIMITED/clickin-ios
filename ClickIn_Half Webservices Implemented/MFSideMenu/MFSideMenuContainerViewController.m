...
+ (MFSideMenuContainerViewController *)containerWithCenterViewController:(id)centerViewController
    ...
    controller.rightMenuViewController = rightMenuViewController; // rightMenuViewController replaced to disable right menu
    return controller;
}
...
- (void)toggleRightSideMenuCompletion:(void (^)(void))completion {
    if(self.menuState == MFSideMenuStateRightMenuOpen) {
        ...
        //custom webservice call for fetching notifications
        [[NSNotificationCenter defaultCenter] postNotificationName:Notification_RightMenuToggled object:nil userInfo:Nil];
    }
}
    ...
    switch (menuState) {
        case MFSideMenuStateClosed: {
            ...
            [self closeSideMenuCompletion:^{
                ...
                innerCompletion();
                //custom method for viewDidDisappear  ---amit
                [self callViewDidDisappear];
                [self centerMenuWillShow];
            }];
            break;
        }
...
- (void)rightMenuWillShow {
    ...
    [self.rightMenuViewController viewWillAppear:YES];
    [self.menuContainerView bringSubviewToFront:[self.rightMenuViewController view]];
}

//center view will appear
- (void)centerMenuWillShow
{
    [self.centerViewController viewWillAppear:YES];
   [self.leftMenuViewController viewDidDisappear:YES];
}


//custom method for viewDidDisapper  --amit
-(void)callViewDidDisappear
{
    [self.rightMenuViewController viewDidDisappear:YES];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    return YES;   /// for disabling/enabling touch gestures for navigating left and right
...
- (void) handlePan:(UIPanGestureRecognizer *)recognizer {
    ...
    // code to restrict the pan when center view is open
    if((self.menuState != MFSideMenuStateRightMenuOpen )
